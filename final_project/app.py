...
# 2. Custom CSS for Decluttered / Publication-Ready Styling
st.markdown(
    """
    <style>
...
# 3. Robust Data Loader (Handles path automatically without needing 'os')
@st.cache_data
def load_data():
    current_dir = Path(__file__).resolve().parent

    # Dynamically search for the CSV file relative to app.py
    possible_paths = [
        current_dir / "Food_Delivery_Times.csv",
        current_dir / "data" / "Food_Delivery_Times.csv",
        current_dir.parent / "Food_Delivery_Times.csv",
    ]
    ...
    if file_path is None:
        st.error("Could not find Food_Delivery_Times.csv in project directory.")
        st.stop()

    df = pd.read_csv(file_path)

    # Clean Missing Values
    if "Weather" in df.columns and not df["Weather"].dropna().empty:
        df["Weather"] = df["Weather"].fillna(df["Weather"].mode()[0])
    if "Traffic_Level" in df.columns and not df["Traffic_Level"].dropna().empty:
        df["Traffic_Level"] = df["Traffic_Level"].fillna(
            df["Traffic_Level"].mode()[0]
        )
    if "Time_of_Day" in df.columns and not df["Time_of_Day"].dropna().empty:
    ...
    # Feature Engineering
    df["Speed_kmh"] = df["Distance_km"] / (
        (df["Delivery_Time_min"] - df["Preparation_Time_min"]) / 60
    )

    df["Distance_Category"] = pd.cut(
        df["Distance_km"],
        bins=[0, 5, 12, 25],
        labels=["Short (<5km)", "Medium (5-12km)", "Long (>12km)"],
    )

    df["Exp_Group"] = pd.cut(
        df["Courier_Experience_yrs"],
        bins=[-1, 2, 5, 10],
        labels=["Novice (0-2y)", "Mid (3-5y)", "Senior (5y+)"],
    )
...
st.markdown(
    "An interactive operational dashboard exploring drivers of delivery duration, vehicle efficiency, and weather/traffic bottlenecks."
)
...
m2.metric(
    "Avg Delivery Time", f"{filtered_df['Delivery_Time_min'].mean():.1f} min"
)
m3.metric("Avg Distance", f"{filtered_df['Distance_km'].mean():.1f} km")
m4.metric(
    "Avg Prep Time", f"{filtered_df['Preparation_Time_min'].mean():.1f} min"
)
...
tab1, tab2, tab3 = st.tabs(
    [
        ...
        "🌧️ Weather & Traffic Impact",
        "👨‍✈️ Courier & Kitchen Dynamics",
    ]
)
...
with tab1:
    ...
    with col2:
        df_speed = (
            filtered_df.groupby(["Distance_Category", "Vehicle_Type"])[
                "Speed_kmh"
            ]
            .mean()
            .reset_index()
...
with tab2:
    ...
    with col3:
        df_wt = (
            filtered_df.groupby(["Weather", "Traffic_Level"])[
                "Delivery_Time_min"
            ]
            .mean()
            .reset_index()
...
with tab3:
    ...
    with col5:
        fig5 = px.box(
            ...
            labels={
                "Courier_Experience_yrs": "Experience (Years)",
                "Delivery_Time_min": "Delivery Time (min)",
            },
        )
        fig5.update_layout(
    ...
    with col6:
        fig6 = px.scatter(
            ...
            labels={
                ...
                "Delivery_Time_min": "Total Delivery Time (min)",
            },
        )
        fig6.update_layout(
            xaxis=dict(showgrid=False),
...
st.markdown("---")
st.caption(
    "Food Delivery Performance Analysis Dashboard • Developed with Streamlit & Plotly"
)
